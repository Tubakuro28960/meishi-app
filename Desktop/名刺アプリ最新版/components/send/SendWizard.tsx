          ...
          return (
            <label key={tmpl.id} style={{ ...s.tmplRow, ...(checked ? s.tmplRowChecked : {}) }}>
              ...
              <div style={s.tmplInfo}>
                ...
                <span style={s.tmplSubject}>
                  件名: {tmpl.subject_template || "（未設定）"}
                </span>
              </div>
            </label>
          );
...
function Step3({
  ...
  return (
    <div>
      ...
      {/* メール方法切り替え */}
      <div style={s.modeRow}>
        <span style={s.modeLabel}>開く方法:</span>
        {(["mailto", "gmail"] as MailMode[]).map((m) => (
          <label key={m} style={s.modeOption}>
            <input
              type="radio"
              name="mailMode"
              checked={mailMode === m}
              onChange={() => onMailModeChange(m)}
            />
            {m === "mailto" ? "メールアプリ (mailto)" : "Gmail (Web)"}
          </label>
        ))}
      </div>
      ...
      <div style={s.previewList}>
        {previews.map((p, i) => {
          const url = buildUrl(mailMode, p.card.email!, p.subject, p.body);
          return (
      ...
      {previews.length > 1 && (
        <div style={s.openAllBox}>
          <button onClick={handleOpenAll} style={s.openAllBtn}>
            全 {previews.length} 件を順番に開く
          </button>
          <p style={s.popupWarn}>
            ※ ブラウザのポップアップブロックが有効だと2件目以降が開かない場合があります。
            その場合は1件ずつ「メール作成を開く」を押してください。
          </p>
        </div>
      )}
...
export default function SendWizard({ cards, templates }: Props) {
  ...
  const previews: Preview[] = selectedCards
    ...
    .map((card) => {
      ...
      return {
        card,
        subject: selectedTemplate ? renderTemplate(selectedTemplate.subject_template, vars) : "",
        body:    selectedTemplate ? renderTemplate(selectedTemplate.body_template, vars) : "",
      };
    });
  ...
  async function handleRecord() {
    ...
    const jobInputs = previews.map((p) => ({
...
const s: Record<string, React.CSSProperties> = {
  ...
  previewTo:      { fontSize: "0.8125rem", color: "#64748b", fontFamily: "monospace" },
